...
class SatModel:
    ...
    def _get_cipher_inputs_values(self, out_suffix, output_values_dict):
        components_values = {}
    ...
    def _solve_with_external_sat_solver(self, model_type, solver_name, options, host=None, env_vars_string=""):
        ...
        if status == 'SATISFIABLE':
            output_values_dict = self._get_solver_solution_parsed(variable2number, values)
            component2fields, total_weight = self._parse_solver_output(output_values_dict)
        else:
